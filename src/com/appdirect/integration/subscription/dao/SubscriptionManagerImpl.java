...
public class SubscriptionManagerImpl extends AppDAOManagerImpl implements SubscriptionManager {
	...
	public Response SubscriptionNotice(SubscriptionNotice order) throws Exception {
		...
		try {
			...
			Order tempOrder = new Order();
			Order resultOrder = null;
			tempOrder.setOrderTypeId(OrderType.SUBSCRIPTION_NOTICE.getId());
			validateAndSetMarketPlaceOnOrder(tempOrder, order);
			Payload payload = order.getPayload();
			if(payload!= null && payload.getAccount()!= null) {
				Account account = accountDAO.getObject(payload.getAccount().getAccountIdentifier(), jTemplate);
				if(account== null) {
					...
					resultOrder = orderDAO.add(tempOrder, jTemplate);
					if(resultOrder!= null) {
						Account resultAccount = updateAccountByNoticeType(account, payload.getNotice());
	...
	private Account updateAccountByNoticeType(Account account, Notice notice) throws Exception {
		Account tempAccount = new Account(account);
		Account resultAccount = null;
		if(notice.getType().equals(NoticeType.CLOSED.name())) {
			tempAccount.setStatus(statusCode.CANCELLED.getCode());
			resultAccount = accountDAO.update(tempAccount, jTemplate);
		}else if(notice.getType().equals(NoticeType.REACTIVATED.name())) {
			tempAccount.setStatus(statusCode.ACTIVE.getCode());
			resultAccount = accountDAO.update(tempAccount, jTemplate);
		}else if(notice.getType().equals(NoticeType.DEACTIVATED.name())) {
			tempAccount.setStatus(statusCode.SUSPENDED.getCode());
			resultAccount = accountDAO.update(tempAccount, jTemplate);
		}else if(notice.getType().equals(NoticeType.UPCOMING_INVOICE.name())) {
			resultAccount = tempAccount; // no change for upcoming invoice
		} else {
			throw new Exception("Unknow notice type in subscrption notice");
		}
